...
: "${SINCE:=$(date -d "90 days ago" +%Y%m%d 2>/dev/null || date -v-90d +%Y%m%d 2>/dev/null)}"
...
python backend/ops_sync.py predictions --since "$SINCE" --max-races 2500 --model-version heuristic-place-1.3
